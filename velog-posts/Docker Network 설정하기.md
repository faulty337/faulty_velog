...
기존의 테스트 방식은 k6의 run script.js이며, 부하테스트의 경우 실제 서비를 돌려보는 것이기에 토큰정보 등이 redis에 그대로 남아 있을 수 있기 때문에 flushall을 이용해 초기화를 해줘야 한다.
...
3. 문제 추적 : 분리되어 있게 되면 문제 발생시 이에 대한 추적해야할 트래픽을 줄이고 요이하게 해준다.
